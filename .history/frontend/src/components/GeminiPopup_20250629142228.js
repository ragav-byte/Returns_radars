...
import { ref, push, serverTimestamp } from "firebase/database"; // ✅ import needed functions
...
function GeminiPopup({ data, onClose }) {
  ...
  useEffect(() => {
    const saveToRealtimeDB = async () => {
      try {
        await push(ref(realtimeDB, "popupHistory"), {
          ...data,
          timestamp: serverTimestamp()
        });
      } catch (error) {
        console.error("Error saving popup data to Realtime DB:", error);
      }
    };
